...
void SpriteFont::draw(SpriteBatch& batch, const char* s, glm::vec2 position, glm::vec2 scaling,
	float depth, Colour tint, Justification just /* = Justification::LEFT */) {
	...
	for (int si = 0; s[si] != 0; si++) {
		...
		if (s[si] == '\n') {
			...
			batch.addSprite(glm::vec2(destRect.x, destRect.y), glm::vec2(destRect.z, destRect.w), _glyphs[gi].uvRect, _texID, depth, tint);
